...
const App = () => {
  ...
  return (
    <>
      {componentRunning ? (
        <button onClick={handleComponentToggle} className="stop bt">
          컴포넌트 정지
        </button>
      ) : (
        <button onClick={handleComponentToggle} className="start bt">
          컴포넌트 시작
        </button>
      )}
      <div className="container">{componentRunning ? <Clock /> : <></>}</div>
    </>
  );
};
